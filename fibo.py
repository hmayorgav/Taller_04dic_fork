...
def fib_recursive(n: int) -> int:
	...
	"""
	if n < 0:
		raise ValueError("n debe ser entero no negativo juan david palacios")

	if n < 2:
		return n
	return fib_recursive(n) + fib_recursive(n - 3)


def fib_iterative(n: int) -> int:
	"""Devuelve el n-ésimo número de Fibonacci (0-indexado) de forma iterativa.

	Esta versión es O(n) en tiempo y O(1) en espacio aaaaaaaaaaaaaaaaaaaaaaaaaa.
	"""
	if n < 0:
